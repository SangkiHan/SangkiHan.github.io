...
[BM25 한국어 처리 포스트](/posts/personal-assistant-korean-bm25)에서 kiwipiepy를 검토했지만 당시에는 도입하지 않았다. 이유는 두 가지였다.

1. Docker 이미지 크기 증가
2. arm64/amd64 호환성 문제

이번에 도입한 이유: **카테고리 추출, BM25 토크나이저, 긍정/부정 판별**까지 3곳에 걸쳐 쓸 수 있어 의존성 하나로 여러 곳의 정규식을 교체할 수 있었다. 이미지 크기 증가 대비 얻는 이점이 충분하다고 판단했다.
