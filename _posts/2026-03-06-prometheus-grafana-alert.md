...
이번 글에서는 **JVM 힙 메모리 사용률이 80%를 초과하면 Slack으로 알림을 발송**하는 Grafana Alert를 설정합니다.

GC(Garbage Collection)가 발생했음에도 불구하고 힙 메모리 점유율이 80%를 넘는다는 것은 두 가지 중 하나를 의미합니다.
...
```promql
(jvm_memory_used_bytes{job="puppynote-dev", area="heap"} / jvm_memory_max_bytes{job="puppynote-dev", area="heap"}) * 100
```
puppynote-dev는 사용자가 Prometheus scrape 시 설정한 job 레이블 값입니다. 실제 환경에 맞게 수정해야 합니다.

이 쿼리는 힙 전체 대비 현재 사용 중인 메모리의 비율(%)을 계산합니다.
`application` 레이블이 설정되어 있으면 dev/prd 환경별로 분리된 시계열이 반환됩니다.
...
GC가 일시적으로 급증한 것인지, 아니면 지속적으로 80%를 넘고 있는지 구분하는 단계입니다.
